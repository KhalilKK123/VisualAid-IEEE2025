...
#     "mysql+pymysql://root:@localhost:3306/visualaiddb"
# )
...
def handle_object_detection(data):
    try:
        ...
        detections = (
            results.pandas().xyxy[0][["name", "confidence"]].to_dict(orient="records")
        )
...
def testlol(data):
    try:
        print(data)
        socketio.emit("test-result", str(data) + ": success")
    except Exception as e:
        print("error4 " + e)
        socketio.emit("test-result", {"success": False, "error": str(e)})


# Test socket 2
@socketio.on("what-time")
def whatTime(data):
    try:
        print(data)
        socketio.emit("time-is", data)
    except Exception as e:
        print("error5 " + str(e))
        socketio.emit("time-is", {"success": False, "error": str(e)})
...
#                     {
#                         "success": False,
#                         "message": "Email and customization are required",
#                     }
#                 ),
#                 400,
#             )
...
#         return (
#             jsonify({"success": True, "message": "Customization updated successfully"}),
#             200,
#         )
...
#             return (
#                 jsonify(
#                     {"success": False, "message": f"No user found with email: {email}"}
#                 ),
#                 404,
#             )
...
#         return (
#             jsonify({"success": True, "message": "Test user added successfully"}),
#             201,
#         )
...
#         return (
#             jsonify({"success": False, "message": f"Error adding test user: {str(e)}"}),
#             500,
#         )
