...
class WebSocketService {
    ...
    try {
      ...
      _socket!.onConnect((_) {
        ...
        _responseController.add({
          'event': 'connect',
          'result': {'status': 'connected', 'id': _socket?.id}
        });
      });
      ...
      _socket!.onConnectError((data) {
        ...
        _responseController.add({
          'event': 'connect_error',
          'message': 'Connection Error: ${data ?? "Unknown"}'
        });
      ...
      _socket!.on('connect_timeout', (data) {
        ...
        _isConnecting = false;
        _responseController
            .add({'event': 'connect_timeout', 'message': 'Connection Timeout'});
        _scheduleReconnect(isInitialFailure: true);
      });
      ...
      _socket!.onError((data) {
        ...
        _responseController.add({
          'event': 'error',
          'message': 'Socket Error: ${data ?? "Unknown"}'
        });
      ...
      _socket!.on('response', (responseData) {
        ...
        try {
          if (responseData is Map<String, dynamic>) {
            _responseController
                .add(responseData); // Pass the whole map directly to HomeScreen
          } else if (responseData != null) {
            debugPrint(
                '[WebSocketService] Received unexpected non-map data on "response": $responseData');
            _responseController.add({
          ...
          _responseController.add({
            'event': 'error',
            'message': 'Client-side Data Processing Error: $e'
          });
        }
      });
      // --- --- --- --- --- --- --- ---
      ...
      _socket!.on('reconnect_failed', (data) {
        debugPrint(
            '[WebSocketService] Reconnect failed permanently (after max attempts): $data');
        _responseController.add({
          'event': 'reconnect_failed',
          'message': 'Reconnect Failed Permanently'
        });
      });
      // --- --- --- --- --- --- --- ---
  ...
  void sendImageForProcessing({
    ...
    if (!isConnected || _socket == null) {
      debugPrint(
          '[WebSocketService] Cannot send image for $processingType: Not connected.');
      _responseController
          .add({'event': 'error', 'message': 'Cannot send: Not connected'});
      return;
    }

    String logDetails = "";
    if (processingType == 'text_detection' && languageCode != null)
      logDetails += "(Lang: $languageCode)";
    ...
    if (processingType == 'supervision' && supervisionRequestType != null)
      logDetails += " (Request: $supervisionRequestType)";
    debugPrint(
        '[WebSocketService] Preparing image for $processingType $logDetails...');
    ...
    try {
      ...
      final payload = <String, dynamic>{
        'type':
            processingType, // This is the main 'type' the backend uses for routing
        'image': base64Image,
        ...
        if (processingType == 'supervision' && supervisionRequestType != null)
          'request_type':
              supervisionRequestType, // Backend expects 'request_type'
      };
      ...
      debugPrint(
          '[WebSocketService] Error preparing/sending image for $processingType: $e');
      debugPrintStack(
          stackTrace: stackTrace,
          label: '[WebSocketService] Send Image Error StackTrace');
      _responseController.add(
          {'event': 'error', 'message': 'Image send failed: ${e.toString()}'});
    }
  }
  ...
  void _scheduleReconnect({bool isInitialFailure = false}) {
    ...
    debugPrint(
        '[WebSocketService] Scheduling Socket.IO reconnect attempt in $currentDelay seconds...');
